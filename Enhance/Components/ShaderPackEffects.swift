// Vendored from SwiftUIShaders by Kris Puckett (https://github.com/krispuckett/SwiftUIShaders),
// MIT licensed — see THIRD_PARTY_NOTICES.md. Two adaptations for the app target, both mechanical:
// `ShaderLibrary.bundle(.module)` became bare `ShaderLibrary` (the shaders compile into the app's
// own default.metallib, exactly like `ShaderLibrary.pixellate` in GradientViews), and the
// extension dropped `public` (there is no module boundary to cross). Everything else is verbatim,
// so a diff against upstream stays readable when new shaders land there.
//
...
extension View {
    ...
    func bcsKaleidoscope(segments: Float = 9, rotation: Float = 3.14, zoom: Float = 1.75, animateSpeed: Float = 1, maxSampleOffset: CGSize = CGSize(width: 64, height: 64)) -> some View {
        modifier(_AnimatedShaderEffect(maxSampleOffset: maxSampleOffset) { size, t in
            ShaderLibrary.bcs_kaleidoscope(
                .float2(size),
                .float(t),
                .float(segments),
                .float(rotation),
    ...
    func bcsLiquidChrome(distortion: Float = 15, chromeIntensity: Float = 0.5, flowSpeed: Float = 1.55, reflectionScale: Float = 5.5, maxSampleOffset: CGSize = CGSize(width: 64, height: 64)) -> some View {
        modifier(_AnimatedShaderEffect(maxSampleOffset: maxSampleOffset) { size, t in
            ShaderLibrary.bcs_liquidChrome(
                .float2(size),
                .float(t),
                .float(distortion),
    ...
    func bcsMorphBreathe(breatheDepth: Float = 28, breatheRate: Float = 1.65, warpComplexity: Float = 4.5, organic: Float = 0.5, maxSampleOffset: CGSize = CGSize(width: 64, height: 64)) -> some View {
        modifier(_AnimatedShaderEffect(maxSampleOffset: maxSampleOffset) { size, t in
            ShaderLibrary.bcs_morphBreathe(
                .float2(size),
                .float(t),
                .float(breatheDepth),
                .float(breatheRate),
    ...
    func bcsPixelateStorm(pixelSize: Float = 21, stormAmount: Float = 0.5, swirl: Float = 1.5, pulse: Float = 1.5, maxSampleOffset: CGSize = CGSize(width: 64, height: 64)) -> some View {
        modifier(_AnimatedShaderEffect(maxSampleOffset: maxSampleOffset) { size, t in
            ShaderLibrary.bcs_pixelateStorm(
                .float2(size),
                .float(t),
                .float(pixelSize),
                .float(stormAmount),
