...
struct FrameEchoEffect: VisualEffect {
    /// The nearest echo's opacity.
    private let opacity: CGFloat
    /// Ratio between consecutive echoes: echo `k` draws at `opacity · fade^(k-1)`. 1 never
    /// fades, so a trail persists across the whole burst *(user's ask, 2026-09-03)*.
    ...
    /// - Parameters:
    ///   - intensity: the FADE slider — how much each echo keeps of the last (0.3…1.0).
